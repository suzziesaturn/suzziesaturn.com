...
function NotifyBtn({ product }: { product: string }) {
  ...
  if (!open) return (
    <button
      ...
      className="w-fit border border-[#ccc] px-[10px] py-1 font-sans text-[9px] font-bold uppercase tracking-[0.2em] text-[#999] hover:border-black hover:text-black transition-colors"
    >
      Notify Me
    </button>
  );
  ...
  return (
    <div className="flex border border-black">
      <input
        ...
        className="min-w-0 flex-1 bg-transparent px-2 py-1 font-sans text-[9px] outline-none placeholder:text-black/30"
      />
      <button
