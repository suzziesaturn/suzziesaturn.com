      ...
      <div className="px-5 pb-10 pt-16 sm:px-8 sm:pt-24 sm:pb-14">
        <div className="grid grid-cols-1 gap-12 sm:grid-cols-[2fr_1fr_1fr_1fr]">
          <div className="max-w-sm">
            ...
            {status === "done" ? (
              <p className="font-sans text-sm italic text-white/60">You're on the list.</p>
            ) : (
              <div className="flex w-full border border-white/25">
                <input type="email" placeholder="Your email" value={email} onChange={e => setEmail(e.target.value)} className="min-w-0 flex-1 bg-transparent px-4 py-3 font-sans text-[11px] font-medium tracking-wide text-white placeholder:uppercase placeholder:tracking-widest placeholder:text-white/35 outline-none" />
                <button type="button" onClick={handleJoin} disabled={status === "loading"} className="shrink-0 bg-white px-4 py-3 font-sans text-[10px] font-bold uppercase tracking-widest text-black transition-opacity hover:opacity-80 disabled:opacity-40">
                  {status === "loading" ? "..." : "Join"}
                </button>
              </div>
            )}
