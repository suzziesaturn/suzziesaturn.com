      ...
      <main className="min-h-screen px-5 py-16 sm:px-8 sm:py-24 max-w-3xl mx-auto">
        ...
        <form className="flex flex-col gap-6">
          <div className="flex flex-col gap-2">
            ...
            <input type="text" placeholder="Your name" className="border border-black/20 px-4 py-3 font-sans text-sm outline-none focus:border-black transition-colors" />
          </div>
          <div className="flex flex-col gap-2">
            <label className="font-sans text-[10px] font-bold uppercase tracking-[0.2em]">Email</label>
            <input type="email" placeholder="your@email.com" className="border border-black/20 px-4 py-3 font-sans text-sm outline-none focus:border-black transition-colors" />
          </div>
          <div className="flex flex-col gap-2">
            <label className="font-sans text-[10px] font-bold uppercase tracking-[0.2em]">Order Number (optional)</label>
            <input type="text" placeholder="#12345" className="border border-black/20 px-4 py-3 font-sans text-sm outline-none focus:border-black transition-colors" />
          </div>
          <div className="flex flex-col gap-2">
            <label className="font-sans text-[10px] font-bold uppercase tracking-[0.2em]">Message</label>
            <textarea rows={6} placeholder="What's on your mind?" className="border border-black/20 px-4 py-3 font-sans text-sm outline-none focus:border-black transition-colors resize-none" />
          </div>
          <button type="submit" className="bg-black text-white font-sans text-xs font-bold uppercase tracking-[0.25em] py-4 px-8 hover:opacity-80 transition-opacity w-fit">Send Message</button>
        </form>
